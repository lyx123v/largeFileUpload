...
export const calHash = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = e => {
      const { hash, percentage } = e.data; // 从 Worker 接收到的哈希值和计算进度
      const hashPercentage = parseInt(percentage.toFixed(2)); // 将计算进度转换为整数百分比
      onTick?.(hashPercentage); // 调用 onTick 回调，如果存在的话
      if (hash) {
